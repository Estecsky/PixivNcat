...
Pixiv_help_info =[
    ...
    # "4.输入 /pixiv close ，bot会自动关闭。",
    # "5.输入 /pixiv open ，bot会自动开启。",
    # "6.输入 /pixiv test ，bot会自动发送测试消息。",
    ...
    async def on_group_event(self, msg: GroupMessage):
        ...
        if PixivNcat_on == True:
            if msg.raw_message.lower() == "/pixiv 随机涩图" or msg.raw_message.lower() == "随机涩图":
                selceted_img_name = get_Ramdom_imgId(images_path)
                
                selceted_file_path = os.path.join(images_path, selceted_img_name)
                if os.path.exists(selceted_file_path) == False:
            ...
            if msg.raw_message.lower().startswith("/pixiv pid "):
                artwork_pid = msg.message["data"]["text"].strip()[10:] #/pixiv pid 129105117
                if artwork_pid.isdigit() == False:
                    ...
                    if "is_R18" in artwork_result:
                        ...
                        if file_name is None:
                            ...
                            await self.api.post_group_msg(msg.group_id,rtf=msg_chain,reply=msg.message_id)
